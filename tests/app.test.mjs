...
test('full app flow: import PGN, tag a line, render table preview', async () => {
  const dom = new JSDOM('<!DOCTYPE html><main id="view"></main>', { url: 'http://localhost/', pretendToBeVisual: true });
  ...
  const loadBtn = [...view.querySelectorAll('button')].find((b) => b.textContent.includes('Load'));
  loadBtn.click();
  ...
  const minorBtn = [...secondRow.querySelectorAll('button')].find((b) => b.textContent === 'Minor');
  minorBtn.click();
  // preview should now carry a "minor" tag label
  const view2 = doc('view');
  const preview = view2.querySelector('table.tbl');
  assert.ok(preview.textContent.includes('minor'), 'minor tag appears in table');
